...
import { MinusIcon, TrendUpIcon, ArrowUpIcon, CaretUpIcon, TrendDownIcon, ArrowDownIcon, CaretDownIcon } from "@phosphor-icons/react";
			...
			{variant === "badge" ? (
				...
				<div
					className={cn(
						...
						value > 0 ? "text-success-soft-foreground" : "",
						value < 0 ? "text-destructive-soft-foreground" : "",
						className
					)}
					data-slot="delta"
					{...props}
	...
	return (
		<span
			className={cn(
				...
				"[&_svg]:size-2! [&_svg]:shrink-0 [&_svg]:stroke-[1px]! [&_svg]:text-background",
				value > 0 && "bg-success",
...
function DeltaIcon({
	...
	if (resolvedValue > 0) {
		if (variant === "trend") {
			return shell(
				<TrendUpIcon {...slotProps} className={mergedClassName} {...props} />
			);
		}
		...
		return shell(
			<CaretUpIcon {...slotProps} className={mergedClassName} {...props} />
		);
	}

	if (variant === "trend") {
		return shell(
			<TrendDownIcon {...slotProps} className={mergedClassName} {...props} />
		);
	}
	...
	return shell(
		<CaretDownIcon {...slotProps} className={mergedClassName} {...props} />
	);
}
